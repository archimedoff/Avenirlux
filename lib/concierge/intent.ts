...
const EXTRA_CITIES = ["Kyoto", "Rome", "Los Angeles", "Hong Kong", "Istanbul", "Marrakech", "Cape Town", "Sydney", "Amsterdam", "Zurich"] as const;
...
export function parseIntent(message: string, modeOverride?: TripMode): ParsedIntent {
  ...
  if (!modeOverride) {
    for (const { mode: m, pattern } of MODE_PATTERNS) {
      if (pattern.test(text)) { mode = m; break; }
    }
  }
  let city: string | undefined;
  for (const name of ALL_CITIES) {
    if (lower.includes(name.toLowerCase())) { city = name; break; }
  }
  const wantsHotels = /\b(hotel|stay|residence|suite|villa|resort|book|room)\b/i.test(text) || Boolean(city);
  const wantsItinerary = /\b(itinerary|plan|weekend|days|schedule|trip)\b/i.test(text);
  const wantsHiddenGems = /\b(hidden gem|secret|local|under the radar)\b/i.test(text) || wantsItinerary;
  return { city, mode, wantsHotels, wantsItinerary, wantsHiddenGems };
}
