...
export function AuthModal({ open, mode, onClose, onModeChange }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="auth-modal-title">
      <button type="button" className="absolute inset-0 bg-[var(--luxury-ink)]/55 backdrop-blur-md" aria-label="Close" onClick={onClose} />
      <div className="auth-panel page-enter relative z-[1] w-full max-w-md overflow-hidden rounded-t-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-float)] sm:rounded-[var(--radius-card)]">
        ...
        <div className="relative p-6 sm:p-8">
          ...
          <div className="mb-6 flex rounded-full bg-[var(--surface-muted)] p-1">
            <button
              type="button"
              className={`flex-1 rounded-full py-2 text-sm font-medium transition-all duration-300 ${mode === "signin" ? "bg-white text-[var(--foreground)] shadow-sm" : "text-[var(--foreground-muted)]"}`}
            ...
            <button
              type="button"
              className={`flex-1 rounded-full py-2 text-sm font-medium transition-all duration-300 ${mode === "signup" ? "bg-white text-[var(--foreground)] shadow-sm" : "text-[var(--foreground-muted)]"}`}
              onClick={() => onModeChange("signup")}
            >
              Sign up
            </button>
          </div>
          {mode === "signin" ? <SignInForm onSuccess={onClose} /> : <SignUpForm onSuccess={onClose} onSwitch={() => onModeChange("signin")} />}
        </div>
      </div>
    </div>
  );
